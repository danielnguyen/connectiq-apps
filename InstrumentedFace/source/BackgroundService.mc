...
class BackgroundService extends System.ServiceDelegate {
    ...
    private function makeRequest_OpenCovid() {
        ...
        if ($.DEV_MODE) {
            ...
            onReceive_OpenCovid_ON(200, $.TESTDATA_OPENCOVID_ON);
            onReceive_OpenCovid_TOR(200, $.TESTDATA_OPENCOVID_TOR);
            onReceive_OpenCovid_YORK(200, $.TESTDATA_OPENCOVID_YORK);
            ...
            Communications.makeJsonRequest(
                ...
                options,
                method(:onReceive_OpenCovid_TOR)
            );
            ...
            Communications.makeJsonRequest(
                ...
                options,
                method(:onReceive_OpenCovid_YORK)
            );
        }
    ...
    public function onReceive_OpenCovid_ON(responseCode as Number, data as Dictionary?) as Void {
        $.logMessage("BackgroundService::onReceive_OpenCovid_ON ENTER");
        $.logMessage("Response Code: " + responseCode);
        if (data != null) {
    ...
    public function onReceive_OpenCovid_TOR(responseCode as Number, data as Dictionary?) as Void {
        $.logMessage("BackgroundService::onReceive_OpenCovid_TOR ENTER");
        $.logMessage("Response Code: " + responseCode);
        if (data != null) {
            onReceive_OpenCovid(responseCode, data["summary"][data.size()-1], "covid19data_tor");
        }
        $.logMessage("BackgroundService::onReceive_OpenCovid_TOR EXIT");
       	exitReq();
    }

    public function onReceive_OpenCovid_YORK(responseCode as Number, data as Dictionary?) as Void {
        $.logMessage("BackgroundService::onReceive_OpenCovid_YORK ENTER");
        $.logMessage("Response Code: " + responseCode);
        if (data != null) {
            onReceive_OpenCovid(responseCode, data["summary"][data.size()-1], "covid19data_york");
        }
        $.logMessage("BackgroundService::onReceive_OpenCovid_YORK EXIT");
       	exitReq();
    }

    private function onReceive_OpenCovid(responseCode as Number, data as Dictionary?, location as String) as Void {
